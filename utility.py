...
def calculate_total_distance(routes, px, py, depot):
    ...
    for route in routes:
        for i in range(len(route)):
            if i == 0:
                total_dist += calculate_euclidean_distance(px, py, depot, route[i])
                if i == len(route)-1:
                    total_dist += calculate_euclidean_distance(px, py, route[i], depot)
            elif i == len(route)-1:
                total_dist += calculate_euclidean_distance(px, py, route[i-1], route[i])
                total_dist += calculate_euclidean_distance(px, py, route[i], depot)
            else:
                total_dist += calculate_euclidean_distance(px, py, route[i-1], route[i])
